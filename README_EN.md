...
> A unified weekly research terminal for A-share reversal signals, specific public-fund selection, and cross-crypto upside ranking
...
### 📅 Weekly Stock Advisor (V14 Research Baseline)
...
~5,500 A-shares by turnover (Eastmoney clist, paged by 100)
        ...
        Rank by reversal score, select Top 1-5
                    ↓
        Rank slots: 35/25/20/12/8%; unused slots remain in cash
                    ↓
...
### 🌐 Multi-Asset Weekly Recommendations

- **A-share stocks**: a cash-aware portfolio of up to five large/mid-cap deep-V reversal candidates, with factual LLM explanations.
- **Specific public funds**: compares 34 active-equity C-share products, including Caitong Growth Select Mixed C (021528), using 1-week/1-month/3-month returns, 20/60-day trends, positive-week ratio, volatility, drawdown, and subscription status. Up to five funds are returned.
- **Mainstream cryptocurrencies**: compares 16 liquid USDT spot assets such as BTC, ETH, and SOL using 7/30-day momentum, 20/60-day trends, volume, volatility, and drawdown. Up to three risk-adjusted leaders are returned.

Each asset class has an independent strategy version, cache, and append-only recommendation audit trail. Fund and crypto rankings are deterministic and can be recomputed from public market data without an LLM.
Fund NAV history comes from Eastmoney/Tiantian Fund public data; cryptocurrency candles come from Binance Spot public market data. Cached data is used only as an explicitly identified fallback.

### 🧭 Account-Independent Research Terminal

The frontend does not read or display real brokerage accounts or holdings. All position percentages are model-portfolio research allocations, not the state of a real account.
...
```
┌──────────────────────────────────────────────────┐
│          Next.js 14 Frontend (Research Terminal)  │
│  MarketOverview │ SectorFlow │ StrategyTelemetry  │
│  WeeklyAdvisor: Stock │ Public Fund │ Crypto        │
└───────────────────────┬──────────────────────────┘
                        │  REST API / WebSocket
┌───────────────────────▼──────────────────────────┐
...
│  ├ All A-shares  │                                 │
│  └ Flow/Fund NAV │  LLM: Claude Sonnet 4.6         │
├──────────────────┤  Concurrency: to_thread × 4     │
│  Binance Spot    ├────────────────────────────────┤
│  Public daily K  │  📅 Multi-Asset Weekly Advisor   │
│  16-asset pool   │  Reversal │ Fund │ Crypto        │
│  Local fallback  │  Independent models and stores   │
└──────────────────┴────────────────────────────────┘
```
...
| `/api/weekly-advisor/fund/generate` | POST | Compare specific public funds and generate recommendations |
| `/api/weekly-advisor/fund/latest` | GET | Get the latest specific-fund report |
| `/api/weekly-advisor/crypto/generate` | POST | Compare mainstream cryptocurrencies and generate recommendations |
| `/api/weekly-advisor/crypto/latest` | GET | Get the latest cross-crypto report |
...
| Public-Fund Comparison | — | **Typically <30s** |
| Cross-Crypto Comparison | — | **Typically <15s** |
