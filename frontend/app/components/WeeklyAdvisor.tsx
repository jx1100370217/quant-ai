...
interface WeeklyReport {
  ...
  market_cap_eligible?: number | null
  kline_evaluated?: number | null
  scan_data_complete?: boolean | null
  scan_metrics_version?: string
...
function Top5Table({ stocks, cashPct = 0 }: { stocks: StockRecommendation[], cashPct?: number }) {
  ...
  const delta7dColor = (v: number | null | undefined) => {
    return getMarketTone(v).text
  }

  return (
    <div className="overflow-hidden rounded-2xl border border-cyan-800/30 bg-slate-950/35">
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-cyan-800/40 bg-cyan-900/10">
        <div className="flex items-center space-x-2">
          <TableIcon className="w-4 h-4 text-cyan-400" />
          <span className="text-sm font-semibold text-cyan-400">最多 5 只 · 现金感知组合</span>
        </div>
        <span className="text-xs text-gray-500 font-mono">反转因子 · 反弹≥3.5% ∧ 反转分≥40</span>
      </div>
          ...
          <tbody>
            {stocks.map((s, i) => (
              ...
              >
                ...
                <td className={`px-3 py-2.5 text-right font-mono ${getMarketTone(s.bounce_pct).text}`}>
                  {fmt(s.bounce_pct, 2)}
                </td>
      ...
      <div className="px-4 py-2 bg-gray-900/60 border-t border-gray-800/60 text-[11px] text-gray-500 leading-relaxed">
        ...
        排名槽位为 35/25/20/12/8%；候选不足时不摊满，保留现金 {cashPct.toFixed(0)}%；单股 -6% 为风险线，组合 ≤ -4% 后按次日可成交价退出
      </div>
    </div>
  )
}
...
function StockCard({ stock, index }: StockCardProps) {
  ...
  const stopPct = stock.current_price > 0
    ...
    : 0
  const targetTone = getMarketTone(targetPct)
  const stopTone = getMarketTone(stopPct)
  const fiveDayTone = getMarketTone(stock.decline_5d)
  const FiveDayIcon = fiveDayTone.direction === 'up'
    ? TrendingUp
    : fiveDayTone.direction === 'down' ? TrendingDown : Minus

  return (
    <div className={`rounded-2xl border ${confidenceBg(stock.confidence)} overflow-hidden transition-all duration-300 hover:-translate-y-0.5 hover:shadow-[0_18px_50px_rgba(0,0,0,0.22)]`}>
      {/* 卡片头部 */}
      <div className="p-4">
        <div className="flex items-start justify-between mb-3">
          ...
          <div className={`flex flex-col items-center px-3 py-1.5 rounded-lg border ${confidenceBg(stock.confidence)}`}>
            <span className="text-xs text-gray-500">信号分</span>
        ...
        <div className="flex items-center space-x-2 mb-3 flex-wrap gap-y-1">
          <div className={`flex items-center space-x-1 px-2.5 py-1 rounded-full border ${targetTone.background} ${targetTone.border}`}>
            <Target className={`w-3 h-3 ${targetTone.text}`} />
            <span className="text-xs text-gray-400">目标</span>
            <span className={`text-xs font-mono font-bold ${targetTone.text}`}>{stock.target_price.toFixed(2)}</span>
            <span className={`text-xs ${targetTone.softText}`}>(+{targetPct.toFixed(1)}%)</span>
          </div>
          <div className={`flex items-center space-x-1 px-2.5 py-1 rounded-full border ${stopTone.background} ${stopTone.border}`}>
            <ShieldAlert className={`w-3 h-3 ${stopTone.text}`} />
            <span className="text-xs text-gray-400">止损</span>
            <span className={`text-xs font-mono font-bold ${stopTone.text}`}>{stock.stop_loss_price.toFixed(2)}</span>
            <span className={`text-xs ${stopTone.softText}`}>({stopPct.toFixed(1)}%)</span>
          </div>
        </div>
        ...
        <div className="mb-3">
          <div className="flex items-center justify-between mb-1 text-xs">
            <span className="text-gray-500">量化信号分（非胜率）</span>
            <span className={`font-mono font-bold ${confidenceColor(stock.confidence)}`}>{stock.confidence}/100</span>
          </div>
        ...
        {stock.decline_5d !== undefined && (
          <div className="mb-3">
            <div className="flex items-center space-x-1 text-xs">
              <FiveDayIcon className={`w-3 h-3 ${fiveDayTone.text}`} />
              <span className="text-gray-500">5日涨跌:</span>
              <span className={`font-mono font-bold ${fiveDayTone.text}`}>
...
export default function WeeklyAdvisor() {
  ...
  if (generating) {
    return (
      <div className="quant-panel p-6 lg:p-7">
        <div className="mb-6 flex items-center space-x-2">
          <CalendarDays className="h-5 w-5 text-cyan-400" />
          <h2 className="text-xl font-semibold">周度信号生成器</h2>
        </div>
  ...
  return (
    <div className="quant-panel relative overflow-hidden p-5 sm:p-6 lg:p-7">
      ...
      <div className="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          ...
          <div className="mt-2 flex flex-wrap items-center gap-2.5">
            ...
            {report?.strategy_version && (
              <span className="font-mono text-[9px] tracking-wider text-cyan-600">{report.strategy_version}</span>
            )}
          </div>
        </div>
        <div className="flex items-center gap-2 self-end sm:self-auto">
          ...
          <button
            ...
            className="flex items-center space-x-1.5 rounded-xl border border-cyan-400/25 bg-cyan-400/[0.07] px-4 py-2 text-xs font-semibold text-cyan-300 transition-all hover:border-cyan-300/40 hover:bg-cyan-400/[0.12] disabled:opacity-40"
          >
      ...
      {!loading && !report && !error && (
        <div className="flex flex-col items-center justify-center py-12 text-center">
          ...
          <p className="text-gray-600 text-xs mb-4">点击右上角「生成本周推荐」，筛选符合当前反转规则的观察标的</p>
          <button
            onClick={generateReport}
        ...
        <div className="space-y-4">
          ...
          {report.report_date && (
            <div className="text-xs text-gray-600 font-mono">
              ...
              {report.generated_at ? ` · 生成于 ${report.generated_at.replace('T', ' ')}` : ''}
            </div>
          )}
          ...
          {report.market_summary && (
            <div className="rounded-lg border border-cyan-800/40 bg-cyan-900/5 p-4">
              <div className="flex items-center space-x-2 mb-2">
                ...
                <span className="text-sm font-semibold text-cyan-400">候选特征摘要</span>
              </div>
          ...
          {report.recommendations && report.recommendations.length > 0 && (
            <Top5Table stocks={report.recommendations.slice(0, 5)} cashPct={report.cash_position_pct ?? 0} />
          )}

          {/* 股票/现金仓位 */}
          {report.recommendations && report.recommendations.length > 0 && (
            <div className="grid grid-cols-2 gap-3">
              <div className="rounded-lg border border-cyan-800/40 bg-cyan-900/10 px-4 py-3">
                <div className="text-xs text-gray-500">股票总仓位</div>
                <div className="text-xl font-mono font-bold text-cyan-400">{(report.invested_position_pct ?? 100).toFixed(0)}%</div>
              </div>
              <div className="rounded-lg border border-emerald-800/40 bg-emerald-900/10 px-4 py-3">
                <div className="text-xs text-gray-500">保留现金</div>
                <div className="text-xl font-mono font-bold text-emerald-400">{(report.cash_position_pct ?? 0).toFixed(0)}%</div>
              </div>
            </div>
          )}
          ...
          {(report.total_candidates_scanned || report.reversal_filtered) && (
            <div className="flex items-center justify-center space-x-2 text-xs text-gray-500 py-2 border-t border-gray-800/50 flex-wrap gap-y-1">
              <div className="flex items-center space-x-1">
                ...
                <span>{report.scan_metrics_version === 'actual-v1' ? '实际收到' : '历史扫描口径'}</span>
                <span className="font-mono font-bold text-gray-400">{report.total_candidates_scanned ?? '--'}</span>
                <span>只</span>
                {report.scan_data_complete === false && (
                  <span className="text-yellow-500">（数据未满90%）</span>
                )}
              </div>
              {report.scan_metrics_version === 'actual-v1' && (
                <>
                  <span className="text-gray-700">→</span>
                  <div className="flex items-center space-x-1">
                    <span>大中盘</span>
                    <span className="font-mono font-bold text-gray-400">{report.market_cap_eligible ?? '--'}</span>
                    <span>只</span>
                  </div>
                  <span className="text-gray-700">→</span>
                  <div className="flex items-center space-x-1">
                    <span>K线完整</span>
                    <span className="font-mono font-bold text-gray-400">{report.kline_evaluated ?? '--'}</span>
                    <span>只</span>
                  </div>
                </>
              )}
