...
import { splitMarketCode, toEastmoneySecid } from '../../lib/marketCode'
...
export async function GET(request: Request) {
  ...
  const { code: normalizedCode } = splitMarketCode(code)
  ...
  try {
    ...
    return NextResponse.json({ success: true, name: data.data.name, code: data.data.code || normalizedCode, klines })
  } catch (e: any) {
